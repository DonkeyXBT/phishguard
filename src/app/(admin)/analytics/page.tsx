...
function MiniBar({ value, max, color }: { value: number; max: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-1">
      ...
      <span className="text-xs text-[var(--text-secondary)] w-8 text-right" style={{ fontFamily: 'var(--mono)' }}>{value}</span>
    </div>
  )
}
...
export default function AnalyticsPage() {
  ...
  const summaryCards = [
    { label: 'Total Reports', value: totalReports, icon: BarChart3, accent: 'border-l-[var(--signal)]' },
    { label: 'Critical', value: data.by_level.critical ?? 0, icon: AlertOctagon, accent: 'border-l-[var(--rust)]' },
    { label: 'High', value: data.by_level.high ?? 0, icon: ShieldAlert, accent: 'border-l-[var(--amber)]' },
    { label: 'Low', value: data.by_level.low ?? 0, icon: ShieldCheck, accent: 'border-l-[var(--signal)]' },
  ]
    ...
    <div className="p-8 page-enter">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-[var(--text-heading)]">Attack <em className="not-italic text-[var(--signal)]" style={{ fontFamily: 'var(--serif)' }}>patterns</em></h1>
          <p className="text-[var(--text-tertiary)] text-sm mt-1">Phishing trends and attack patterns</p>
        </div>
        <select value={days} onChange={e => setDays(Number(e.target.value))}
          className="bg-[var(--bg-card)] border border-[var(--border-primary)] text-[var(--text-primary)] text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-[var(--signal-deep)]">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6 stagger-in">
        {summaryCards.map(c => {
          const Icon = c.icon
          return (
            <div key={c.label} className={`bg-[var(--bg-card)] rounded-xl border border-[var(--border-primary)] border-l-[3px] ${c.accent} p-5`}>
      ...
      <div className="grid grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-primary)] p-5">
          <h2 className="font-semibold text-[var(--text-heading)] mb-4">Reports Per Day</h2>
          <div className="flex items-end gap-px h-40">
            {data.reports_per_day.slice(-days).map((r, i) => (
              <div key={i} className="flex-1 flex flex-col justify-end" title={`${r.day.split('T')[0]}: ${r.count}`}>
                <div className="bg-[var(--signal)] rounded-t min-h-[2px] transition-all hover:opacity-80 bar-fill"
                  style={{ height: `${(r.count / maxDaily) * 100}%` }} />
              </div>
            ))}
          </div>
          <div className="flex justify-between text-xs text-[var(--text-tertiary)] mt-2" style={{ fontFamily: 'var(--mono)' }}>
        ...
        {/* Avg score per day */}
        <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-primary)] p-5">
          <h2 className="font-semibold text-[var(--text-heading)] mb-4">Average Risk Score</h2>
          <div className="flex items-end gap-px h-40">
            {data.avg_score_per_day.slice(-days).map((r, i) => (
              <div key={i} className="flex-1 flex flex-col justify-end" title={`${r.day.split('T')[0]}: ${r.avg_score}`}>
                <div className={`rounded-t min-h-[2px] transition-all hover:opacity-80 bar-fill ${r.avg_score >= 60 ? 'bg-[var(--rust)]' : r.avg_score >= 30 ? 'bg-[var(--amber)]' : 'bg-[var(--signal)]'}`}
      ...
      <div className="grid grid-cols-2 gap-6 mb-6">
        {/* Top signals */}
        <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-primary)] p-5">
          <h2 className="font-semibold text-[var(--text-heading)] mb-4">Top Attack Signals</h2>
          <div className="space-y-2">
            {data.top_signals.slice(0, 10).map(s => (
              <div key={s.code} className="flex items-center gap-3">
                <span className="text-xs text-[var(--text-secondary)] w-48 truncate" style={{ fontFamily: 'var(--mono)' }}>{s.code}</span>
                <MiniBar value={s.count} max={maxSignal} color="bg-[var(--rust)]" />
              </div>
            ))}
        ...
        {/* Top sender domains */}
        <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-primary)] p-5">
          <h2 className="font-semibold text-[var(--text-heading)] mb-4">Top Sender Domains</h2>
          <div className="space-y-2">
            {data.top_sender_domains.slice(0, 10).map(d => (
              <div key={d.domain} className="flex items-center gap-3">
                <span className="text-xs text-[var(--text-secondary)] w-48 truncate" style={{ fontFamily: 'var(--mono)' }}>{d.domain}</span>
                <MiniBar value={d.count} max={maxDomain} color="bg-[var(--amber)]" />
              </div>
            ))}
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-primary)] p-5">
          <h2 className="font-semibold text-[var(--text-heading)] mb-4">Report Sources</h2>
          {Object.entries(data.by_source).map(([src, count]) => (
            <div key={src} className="flex items-center justify-between py-1.5 text-sm">
              ...
              <span className="text-[var(--text-primary)] font-semibold" style={{ fontFamily: 'var(--mono)' }}>{count}</span>
            </div>
          ))}
        ...
        <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-primary)] p-5">
          <h2 className="font-semibold text-[var(--text-heading)] mb-4">Recent Critical Reports</h2>
          <div className="space-y-2">
            {data.recent_critical.map(r => (
              <div key={r.id} className="flex items-center justify-between text-sm border-b border-[var(--border-secondary)] pb-2">
                <div className="min-w-0">
                  ...
                  <div className="text-[var(--text-tertiary)] text-xs" style={{ fontFamily: 'var(--mono)' }}>{r.sender ?? 'unknown'}</div>
                </div>
                <span className="bg-[var(--rust)]/10 text-[var(--rust)] border border-[var(--rust)] text-xs px-2 py-0.5 rounded shrink-0 ml-2 font-medium" style={{ fontFamily: 'var(--mono)' }}>{r.riskScore}</span>
              </div>
            ))}
