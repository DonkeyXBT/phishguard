...
import { DashboardStats, EmailReportSummary } from '../types'
...
export default function Dashboard() {
  ...
  const [recent, setRecent] = useState<EmailReportSummary[]>([])

  useEffect(() => {
    adminApi.getStats().then((r) => setStats(r.data))
    adminApi.getQueue({ limit: 5 }).then((r) => setRecent(r.data))
  }, [])
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-500 mt-0.5">
                    Reported by {r.reporter_email} · {formatDistanceToNow(new Date(r.reported_at), { addSuffix: true })}
                  </div>
                </div>
